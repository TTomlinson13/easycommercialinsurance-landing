...
export default function BlogPost() {
  ...
  useEffect(() => {
    ...
    script.textContent = JSON.stringify({
      ...
      author: {
        '@type': 'Organization',
        name: 'EasyCommercialInsurance.com — Tomlinson & Co',
        url: 'https://easycommercialinsurance.com'
      },
      publisher: {
        '@type': 'Organization',
        name: 'EasyCommercialInsurance.com',
        url: 'https://easycommercialinsurance.com',
        logo: {
          '@type': 'ImageObject',
          url: 'https://easycommercialinsurance.com/logo.svg'
        }
      },
      mainEntityOfPage: {
        '@type': 'WebPage',
        '@id': `https://easycommercialinsurance.com/blog/${post.slug}`
      },
    ...
    // Update page title
    document.title = `${post.title} | EasyCommercialInsurance.com`
    ...
    <div className="min-h-screen bg-white">
      ...
      {notFound && !loading && (
        <div className="text-center py-32">
          ...
          <Link to="/blog" className="text-indigo-600 underline hover:text-indigo-800">← Back to Blog</Link>
        </div>
      )}
      ...
      {post && !loading && (
        <>
          {/* Back Link + Hero */}
          <section className="bg-indigo-900 py-10 px-4">
            <div className="max-w-3xl mx-auto">
              <Link to="/blog" className="inline-flex items-center gap-2 text-indigo-300 hover:text-white text-sm font-semibold mb-5 transition">
                ← Back to Blog
              </Link>
              {/* Tags */}
              <div className="flex flex-wrap gap-2 mb-3">
                {post.tags.map(tag => (
                  <span key={tag} className="bg-indigo-700 text-indigo-100 text-xs font-semibold px-3 py-1 rounded-full">{tag}</span>
                ))}
              </div>
          ...
          <section className="py-12 px-4">
            <div className="max-w-3xl mx-auto">
              <div
                className="prose prose-lg prose-headings:text-indigo-900 prose-a:text-indigo-600 prose-strong:text-indigo-900 max-w-none text-gray-700 leading-relaxed
              ...
              <div className="mt-12 bg-indigo-50 border border-indigo-200 rounded-2xl p-6 text-center">
                <h3 className="text-xl font-bold text-indigo-900 mb-2">Questions About Your Florida Business Insurance?</h3>
                <p className="text-indigo-700 mb-4">Our team specializes in Florida commercial insurance. We shop multiple A-rated carriers to find the best coverage at the best price for your business.</p>
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  ...
                  <a
                    ...
                    className="bg-yellow-400 hover:bg-yellow-500 text-indigo-900 font-bold py-3 px-6 rounded-xl transition"
                  >
                    Get Free Quote →
                  </a>
                </div>
              ...
              <div className="mt-8 text-center">
                <Link to="/blog" className="text-indigo-600 hover:text-indigo-800 font-semibold underline text-sm">
                  ← Back to Blog
                </Link>
              </div>
            </div>
